...
export class ResourceOperacao extends BaseHttpController
  ...
  @ApiOperationGet({
    ...
    parameters: { path: { operando: { type: 'enum', format: '[div|mul|som|sub]', required: true } }, query: { val1: { type: 'number' }, val2: { type: 'number' } } },
    responses: {
      200: { description: 'Success', type: SwaggerDefinitionConstant.Response.Type.OBJECT }
    }
  })
  @httpGet('/:operando')
  public async calc (@requestParam('operando') operando: operacoes, @queryParam('val1') operador1: string, @queryParam('val2') operador2: string) {
